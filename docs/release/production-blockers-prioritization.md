...
script. Phase 99B records a successful local fallback-off JWT smoke against
`http://localhost:5230` without documenting secrets, tokens, or patient data.
The P0 blocker is fully closed only after the smoke is run successfully in real
staging and production environments and evidence is recorded.
...
After that, repeat fallback-off smoke in real staging and production environments, then address legal/privacy/retention plus production backup/monitoring.
