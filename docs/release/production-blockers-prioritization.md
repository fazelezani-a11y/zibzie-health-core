...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Secret management and JWT key rotation | Token signing keys or DB credentials could leak, be reused, or become unrotatable. | Production auth cannot be trusted without controlled secrets and rotation. | Phase 98: Production environment and secrets readiness | Infra / config / operator / docs | Deployment target decision, secret store choice | No, design can proceed | Yes | Yes |
